...
from core.api_client import BackendAPIError  # 백엔드 서버와 통신 중 발생한 오류를 처리하기 위해 api_client.py를 가져옵니다.

if not is_logged_in():
    st.switch_page("app_pages/01_start.py")  # 로그인 상태가 아니면 시작 페이지로 이동합니다.
else:
    ...
    if prompt:  # 사용자가 채팅 입력창에 질문을 입력했을 때만 메시지 처리 로직을 실행합니다.
        with st.chat_message("user"):  # 파일, 화면 영역, 로딩 상태처럼 시작과 종료가 있는 작업 범위를 만듭니다.
            st.write(prompt)  # 사용자가 채팅 입력창에 질문을 입력했을 때만 메시지 처리 로직을 실행합니다.
        ...
        except BackendAPIError as error:  # 백엔드 또는 AI 서비스와 통신 중 오류가 발생했을 때 예외 처리 로직을 실행합니다.
