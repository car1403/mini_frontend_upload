...
if not is_logged_in():
    st.switch_page("app_pages/01_start.py")
else:
    ...
    if prompt:  # 사용자가 채팅 입력창에 질문을 입력했을 때만 메시지 처리 로직을 실행합니다.
        ...
        with st.spinner("AI가 답변을 작성 중입니다..."):  # 화면에 로딩 상태를 표시합니다.
            reply = send_message(st.session_state.user_id, prompt)  # 백엔드 또는 AI 서비스가 만든 응답 문자열을 화면 출력용 변수에 저장합니다.
            reply = reply.get("data", "응답이 없습니다.").get("answer", "응답이 없습니다.")  # 백엔드 또는 AI 서비스가 만든 응답 문자열을 화면 출력용 변수에 저장합니다.
